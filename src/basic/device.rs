use super::{storage::Storage, storage::StoragePool, values::AllowedValues};
use crate::{capabilities::DeviceCap, error::Error, filetypes::Filetype, properties::Property};
...
impl MtpDevice {
    ...
    pub fn update_storage(&mut self, sort_by: StorageSort) -> Result<UpdateStorage, Error> {
        unsafe {
            let res = ffi::LIBMTP_Get_Storage(self.inner, sort_by.to_i32().unwrap());
            match res {
                0 => Ok(UpdateStorage::Success),
                1 => Ok(UpdateStorage::OnlyIds),
...
#[cfg(test)]
mod tests {
    use crate::basic::raw::detect_raw_devices;
